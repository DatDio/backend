...
import com.mailshop_dragonvu.service.Web2mProvider;
import com.mailshop_dragonvu.service.Web2mService;
...
/**
 * Web2m Service Implementation
 * Generates VietQR codes and handles webhook verification
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class Web2mServiceImpl implements Web2mService {

    private final Web2mProvider web2mProvider;
    ...
    public String generateQRCodeUrl(Long amount, Long transactionCode) {
        web2mProvider.validateConfiguration();

        String bankCode = web2mProvider.getBankCode();
        String bankBin = BANK_BIN_MAP.getOrDefault(bankCode.toUpperCase(), bankCode);
        String accountNo = web2mProvider.getBankAccount();
        String accountName = web2mProvider.getAccountName();
    ...
    public boolean verifyWebhook(String authorizationHeader) {
        ...
        String accessToken = web2mProvider.getAccessToken();
        if (Strings.isBlank(accessToken)) {
            log.warn("Web2m access token not configured, skipping verification");
        ...
        boolean isValid = accessToken.equals(bearerToken);
        if (!isValid) {
            log.warn("Invalid webhook bearer token");
        }
        return isValid;
    }
